...
export scripts="$Working_DIR/lib/base.d/*.sh"
...
export scripts="$Working_DIR/lib/install.d/*.sh"
for script in $scripts
do
  if [ -f "$script" ]
  then
  verbose_print "loaidng $script"
  source "$script"
  fi
done
...
if [ -f /.dockerenv ]; then
  verbose_print "Running inside a Docker container."
else
  verbose_print "Not running inside a Docker container."
